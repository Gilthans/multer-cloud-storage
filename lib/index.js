...
var MulterGoogleCloudStorage = /** @class */ (function () {
            ...
            if (blobFile !== false) {
                ...
                file.stream
                    ...
                    cb(null, {
                        ...
                        filename: filename,
                        path: "".concat(blobFile.destination).concat(filename),
                        contentType: blob.metadata.contentType,
                        size: blob.metadata.size,
                        uri: "gs://".concat(blob.metadata.bucket, "/").concat(blobFile.destination).concat(filename),
                        linkUrl: "https://storage.googleapis.com/".concat(blob.metadata.bucket, "/").concat(blobFile.destination).concat(filename),
    ...
    MulterGoogleCloudStorage.prototype.getBlobFileReference = function (req, file) {
        ...
        this.getDestination(req, file, function (err, destination) {
            ...
            var escDestination = "";
            escDestination += destination.replace(/^\/+|\/+$/g, "");
            if (escDestination !== "") {
